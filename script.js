/*****************************************************************
 * Malla interactiva 2025 – toggle aprobar ↔ desaprobar + notas  *
 *****************************************************************/
...
const courses = [
  ...
  /* III ----------------------------------------------------- */
  { id:"estadisticas", name:"Estadísticas para la salud", sem:3, pre:["principios"] },
  ...
  /* VIII ---------------------------------------------------- */
  { id:"metodologia",  name:"Metodología de investigación", sem:8, pre:["ecobiometria"] },
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  /* ---- Clic para toggle aprobar/desaprobar ---- */
  ...
  area.value = localStorage.getItem('tm-notes') || '';

  openBtn.onclick  = ()=> modal.style.display = 'flex';
  closeBtn.onclick = ()=> modal.style.display = 'none';
  saveBtn.onclick  = ()=> {
    localStorage.setItem('tm-notes', area.value);
...
function unapproveCascade(id){
  ...
  while(stack.length){
    ...
    courses.forEach(c=>{
      if(done.has(c.id) && c.pre.includes(cur) && unmet(c)){
        stack.push(c.id);
      }
    });
  }
...
function refreshAll(){
  courses.forEach(c=>{
    ...
    if(done.has(c.id)){
      ...
      b.className='badge'; b.textContent='✔'; el.appendChild(b);
    }else if(unmet(c)){
